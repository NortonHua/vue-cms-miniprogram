  ...
  <div>
    <div v-if="!showEdit" class="container">
      ...
      <el-table stripe v-loading="loading" :data="tableData">
        ...
        <el-table-column :show-overflow-tooltip="true" prop="user_id" label="用户id" width="100"></el-table-column>
        <el-table-column :show-overflow-tooltip="true" prop="order_type" label="订单类型" width="100">
          <template slot-scope="scope">{{ scope.row.order_type ? '外卖' : '堂食' }}</template>
        </el-table-column>
        <el-table-column prop="total_count" label="单品数量" width="100"></el-table-column>
        <el-table-column prop="total_price" width="200" label="总价格"></el-table-column>
        <el-table-column prop="placed_time" :formatter="formatDate" min-width="200" label="下单时间"></el-table-column>
        <el-table-column prop="status" min-width="100" label="状态">
          <template v-if="scope.row.status" slot-scope="scope">
            <div class="tags">
              <el-tag :type="computeTagType(scope.row.status)">{{ orderStatus(scope.row.status) }}</el-tag>
              <!--              <el-tag-->
...
export default {
  name: 'order',
  components: { LinSearch, OrderDetail, LinDatePicker },
  methods: {},
  data() {
    return {
      tableData: [],
