          ...
          <el-col :lg="16" :md="20" :sm="24" :xs="24">
            <el-form :model="form" status-icon ref="form" label-width="100px" @submit.native.prevent>
              ...
              <el-form-item label="状态" prop="status">
                <div class="tags">
                  <el-tag :type="computeTagType(form.status)">{{ orderStatus(form.status) }}</el-tag>
                  <!--                  <el-tag style="margin-left:5px;" v-if="form.status === 1" type="danger" size="mini"-->
                  <!--                  >过期</el-tag-->
                  <!--                  >-->
                </div>
              </el-form-item>
              ...
              <el-form-item label="地址" prop="snap_address">
                <div v-if="form.snap_address" class="address">
                  <div class="header">
                    <span class="name">{{ JSON.parse(form.snap_address).userName }}</span>
                    <span class="mobile">{{ JSON.parse(form.snap_address).mobile }}</span>
                  </div>
                  <div class="detail">
                    {{ JSON.parse(form.snap_address).province }} {{ JSON.parse(form.snap_address).city
                    }}{{ JSON.parse(form.snap_address).county }}
                    **************
                    {{ JSON.parse(form.snap_address).detail }}
                  </div>
                </div>
              </el-form-item>

              <el-form-item label="详情" prop="snap_items">
                <el-table v-if="form.snap_items" stripe :data="JSON.parse(form.snap_items)">
                  ...
                  <el-table-column prop="cartItemPrice" label="交易价" min-width="100"></el-table-column>
...
import StickTop from '../../../component/base/sticky-top/sticky-top'
