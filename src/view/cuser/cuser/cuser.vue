  ...
  <div>
    <div v-if="!showEdit" class="container">
      ...
      <transition name="fade">
        <div class="search" v-if="keyword">
          <p class="search-tip">
            ...
            >”，找到<span class="search-num">{{ totalNums }}</span
            >条用户信息
          </p>
      ...
      <el-table stripe v-loading="loading" :data="tableData">
        ...
        <el-table-column prop="wx_profile" label="头像" width="200">
          <template v-if="scope.row.wx_profile" slot-scope="scope">
            <img
              :src="JSON.parse(scope.row.wx_profile).avatarUrl"
              :alt="JSON.parse(scope.row.wx_profile).avatarUrl"
              class="display_img"
            />
          </template>
        </el-table-column>
...
export default {
  components: { UserDetail, LinSearch },
  data() {
    return {
      tableData: [],
      userId: null,
      ...
      currentPage: 1,
      pageCount: 10,
      refreshPagination: true, // 页数增加的时候，因为缓存的缘故需要刷新分页组件
    }
  },
