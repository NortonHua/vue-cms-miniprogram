<template>
  <div class="container">
    <div class="left">
      <div class="l-side">
        <div class="title">{{ month }}月订单数量</div>
        <div class="num-box">
          <div class="shape"></div>
          <div class="num">{{ count }}</div>
          <i class="iconfont icon-jiantou up"></i>
          <div class="percent">+{{ count % 80 }}%</div>
        </div>
      </div>
      <div class="r-side">
...
export default {
  ...
  methods: {
    async getData() {
      const res = await Analysis.getThisMonthOrderCountInfo()
      this.month = res.month_string
      this.count = res.count_info
    },
    circle() {
